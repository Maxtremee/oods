from uuid import UUID

from server.classes.Persistent import Persistent
from server.classes.Class import Class
from server.classes.Reference import Reference
from server.classes.Item import Item


class Index:
    def __init__(self):
        self.classes = {}

    def add(self, obj: Persistent) -> Reference:
        """Add or update object"""
        cls_name = type(obj).__name__
        id = obj.id
        cls = self.classes.get(cls_name)
        if cls:
            cls.add(id, obj)
        else:
            cls = Class()
            cls.add(id, obj)
        self.classes.update({cls_name: cls})
        return Reference(obj.id, cls_name)

    def get(self, id: UUID, cls_name: str = None) -> Item:
        """Get object"""
        if cls_name:
            cls = self.classes.get(cls_name)
            if cls:
                return cls.get(id)
        else:
            for cls in self.classes:
                item = cls.get(id)
                if item:
                    return item

    def remove(self, id: UUID, cls_name: str = None) -> None:
        """Remove object"""
        if cls_name:
            self.classes.get(cls_name).remove(id)
        else:
            for cls in self.classes:
                cls.remove(id)

    # def _remove_from_class(self, id: UUID, cls_name: str):
    #     cls = self.classes.get(cls_name)
    #     if cls:
    #         cls.remove(id)
    #     else:
    #         raise Exception("No such class")

    # def _add_to_class(self, obj: Persistent, path: list):
    #     """Adds object of certain class to corresponding Class object.
        
    #     Also requires a path object which is a list of next occuring attributes names or list/dict/tuple indexes
    #     """
    #     cls_name = type(obj).__name__
    #     id = obj.id
    #     cls = self.classes.get(cls_name)
    #     if cls:
    #         cls.add(id, path)
    #     else:
    #         cls = Class()
    #         cls.add(id, path)
    #     self.classes.update({cls_name: cls})

    # def _get_persistent_attributes(self, obj: Persistent):
    #     attrs = []
    #     for attr in dir(obj):
    #         pass
        
    # def _compare_objs(self, old: Persistent, new: Persistent):
    #     old_attributes = []
    #     new_attributes = []
    #     pass

    # def process_save(self, old: Persistent, path: list, new: Persistent = None):
    #     """Creates or removes paths to object and it's attributes 
    #     (have to inherit from Persistent)
    #     """
    #     self._add_to_class(old, path)

    #     for attr in dir(old):
    #         if not attr.startswith("_"):
    #             attr_obj = getattr(old, attr)
    #             if isinstance(attr_obj, Persistent):
    #                 path_cpy = path.copy()
    #                 path_cpy.append(attr)
    #                 self.process_save(attr_obj, path_cpy)
    #             elif isinstance(attr_obj, dict):
    #                 for item in attr_obj:
    #                     attr_obj_item = attr_obj[item]
    #                     if isinstance(attr_obj_item, Persistent):
    #                         path_cpy = path.copy()
    #                         path_cpy.append({'attr_name': attr, 'index': item})
    #                         self.process_save(attr_obj_item, path_cpy)
    #             elif isinstance(attr_obj, (list, tuple)):
    #                 for item in range(len(attr_obj)):
    #                     attr_obj_item = attr_obj[item]
    #                     if isinstance(attr_obj_item, Persistent):
    #                         path_cpy = path.copy()
    #                         path_cpy.append({'attr_name': attr, 'index': item})
    #                         self.process_save(attr_obj_item, path_cpy)

    # def process_update(self, obj: Persistent):
    #     """Adds or removes paths to atributes of object"""
    #     pass

    # def process_remove(self, obj: Persistent):
    #     """Removes paths to all of object's attributes. 
    #     Then removes path to itself"""

    #     # remove paths to attributes
    #     # objs_to_remove = []
    #     for attr in dir(obj):
    #         if not attr.startswith("_"):
    #             attr_obj = getattr(obj, attr)
    #             if isinstance(attr_obj, Persistent):
    #                 # objs_to_remove.append(attr_obj)
    #                 self.process_remove(attr_obj)
    #             elif isinstance(attr_obj, dict):
    #                 for item in attr_obj:
    #                     attr_obj_item = attr_obj[item]
    #                     if isinstance(attr_obj_item, Persistent):
    #                         # objs_to_remove.append(attr_obj_item)
    #                         self.process_remove(attr_obj_item)
    #             elif isinstance(attr_obj, (list, tuple)):
    #                 for item in range(len(attr_obj)):
    #                     attr_obj_item = attr_obj[item]
    #                     if isinstance(attr_obj_item, Persistent):
    #                         # objs_to_remove.append(attr_obj_item)
    #                         self.process_remove(attr_obj_item)
    #     # for objct in objs_to_remove:
    #     #     cls_name = type(objct).__name__
    #     #     self._remove_from_class(objct.id, cls_name)
    #     self._remove_from_class(obj.id, type(obj).__name__)

    # def get_path_by_id(self, id: UUID):
    #     """Return path to object by ID"""
    #     for cls in self.classes:
    #         paths = self.classes.get(cls).paths
    #         path = paths.get(id)
    #         if path is not None:
    #             return path
    #     return None

    # def get_path_by_id_and_cls_name(self, id: UUID, cls_name: str):
    #     """Return path to object by class name and ID"""
    #     cls_obj = self.classes.get(cls_name)
    #     path = cls_obj.get(id)
    #     return path

    # def get_path_by_obj(self, obj: Persistent):
    #     cls_name = type(obj).__name__
    #     cls_obj = self.classes.get(cls_name)
    #     if cls_obj:
    #         path = cls_obj.get(id)
    #         return path
